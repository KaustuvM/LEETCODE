...
// MARK: LINK - https://leetcode.com/problems/course-schedule/

fileprivate class Solution {
    func canFinish(_ numCourses: Int, _ prerequisites: [[Int]]) -> Bool {
        // data structure to create the dependency graph
        var dGraph = Dictionary<Int, Array<Set<Int>>>()
        ...
        for req in prerequisites {
            if req[0] >= 0 && req[0] < numCourses
                ...
                if var connArr = dGraph[req[1]] {
                    var outBoundConns = connArr[1]
                    outBoundConns.insert(req[0])
                    connArr[1] = outBoundConns
                    dGraph[req[1]] = connArr
                } else {
                    let inboundConns = Set<Int>()
                    var outboundConns = Set<Int>()
                    outboundConns.insert(req[0])
                    let connArr = [inboundConns, outboundConns]
                    dGraph[req[1]] = connArr
                }
                
                // create node 'a' and update inbound connections information
                if var connArr = dGraph[req[0]] {
                    var inboundConns = connArr[0]
                    inboundConns.insert(req[1])
                    connArr[0] = inboundConns
                    dGraph[req[0]] = connArr
                } else {
                    var inboundConns = Set<Int>()
                    let outboundConns = Set<Int>()
                    inboundConns.insert(req[1])
                    let connArr = [inboundConns, outboundConns]
                    dGraph[req[0]] = connArr
                }
        ...
        for (node, connArr) in dGraph {
            let inboundConns = connArr[0]
            if inboundConns.isEmpty {
                obNodes.append(node)
            }
        }
        ...
        while !obNodes.isEmpty {
            ...
            for conn in outboundConns {
                var connArr = dGraph[conn]
                connArr![0].remove(obNode)
                dGraph[conn] = connArr
                if connArr![0].count == 0 {
                    obNodes.append(conn)
                }
            }
        }
        // if the graph is a DAG (Directed Acyclic Graph), then all the nodes should be part of iNodes
        return iNodes.count == dGraph.count
    }
}
